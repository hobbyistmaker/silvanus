...
void ParametricRenderer::execute(DefaultModelingOrientations model_orientation, const Ptr<Component> &component) {
    ...
    auto      view = m_registry.view<Enabled, PanelGroup, JointGroup, PanelExtrusion, JointOrientation, JoinedPanels>();
    ...
    for (auto&[axis, axis_data] : panel_groups) {
        for (auto&[profile, profile_data] : axis_data) {
            for (auto& [position, position_data]: profile_data) {
                ...
                auto &transform = sketch_transforms[model_orientation][axis];
                ...
                for (auto const&[distance, extrusions] : position_data.panels) {
                    ...
                    auto feature = renderSinglePanel(names, sketch, panels[0], model_orientation, position_data.joints);
                ...
                auto end_pos = timeline->markerPosition() - 1;
                if ((end_pos - start_pos) <= 0) { continue; }

                auto timeline_group = timeline->timelineGroups()->add(start_pos, end_pos);
...
auto ParametricRenderer::renderSinglePanel(
    ...
    DefaultModelingOrientations model_orientation,
    JointRenderData &joints
) -> Ptr<ExtrudeFeature> {
    auto extrusion = sketch.extrudeProfile(data.distance, data.offset);
    ...
    for (auto &cut_pairs: cuts) {
        for (auto &cut: cut_pairs) {
...
void ParametricRenderer::renderJointSketches(
    ...
    ExtrusionDistance panel_thickness,
    DefaultModelingOrientations model_orientation,
...
void ParametricRenderer::renderJointSketch(
    ...
    std::string sketch_prefix,
    const AxisFlag &joint_orientation,
    const JointRenderProfileGroup &joint_groups
) {
    auto pairs    = std::vector<CutProfile>{};
    auto profiles = joint_groups.outside;

    for (auto const&[profile, joints]: profiles) {
        if ((profile.joint_type == JointType::Inverse) && (profile.finger_count <= 1)) {
            continue;
        }
        auto names          = concat_names(std::vector<std::string>(joints.names.begin(), joints.names.end()));
        auto profile_name   = panel_name.append(" " + names + " ") + sketch_prefix.append(" Sketch");
        auto pattern_offset = profile.pattern_offset;
        auto finger_width   = profile.finger_width;
